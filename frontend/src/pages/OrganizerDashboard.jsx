 ...
 const OrganizerDashboard = () => {
   ...
   useEffect(() => {
     const fetchDashboardData = async () => {
       try {
         const res = await axios.get(
           `http://localhost:8080/api/events/dashboard/${organizerId}`
         );
    ...
    if (!analytics || analytics.length === 0)
     ...
     const totalSponsorships = analytics.reduce((sum, e) => sum + e.sponsorships, 0);
     const totalTicketRevenue = analytics.reduce((sum, e) => sum + e.ticketRevenue, 0);
     const totalRegisteredUsers = analytics.reduce((sum, e) => sum + e.registeredUsers, 0);
     const totalNetProfit = analytics.reduce((sum, e) => sum + e.netProfit, 0);
     const totalExpenses = analytics.reduce((sum, e) => sum + e.rentCost + e.foodCost, 0);
   ...
   return (
     ...
     <div className="bg-green-50 border border-green-200 p-4 rounded-lg shadow-sm">
      ...
      <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg shadow-sm">
       ...
       {/*Every Event Breakdown */}
       <div className="space-y-6">
         {analytics.map((event) => {
           const totalExpenses = event.rentCost + event.foodCost;
           const profitColor = event.netProfit >= 0 ? "text-green-600" : "text-red-600";
           ...
           return (
             ...
             >
               ...
               {/* Total Expenses with its  sub details */}
               <div className="bg-white border p-3 rounded-lg text-center mb-4 w-full max-w-md mx-auto">
                 ...
                 <div className="flex justify-center mt-2 gap-4">
                   <div className="bg-orange-100 px-3 py-1 rounded text-sm text-gray-700">
                     <span className="font-semibold">Rent:</span> ${event.rentCost.toFixed(2)}
                   </div>
                   <div className="bg-yellow-100 px-3 py-1 rounded text-sm text-gray-700">
                     <span className="font-semibold">Food:</span> ${event.foodCost.toFixed(2)}
                   </div>
                 </div>
               </div>
